...
contract DSAConnectors is DSTest {
    ...
    function testBasicDeposit() public {
        ...
        // get mock tx data
        (string[] memory targets, bytes[] memory data) = buildBasicDeposit();
        ...
        assertTrue(IERC20Minimal(tokenAddress[0]).balanceOf(_account) == amount[0]);
    }

    function testBasicDepositNative() public {
        // DSA creation 
        address _account = instaIndex.build(address(this), 2, address(0)); // 2 is the most recent DSA version
        // get mock tx data
        (string[] memory targets, bytes[] memory data) = buildBasicDepositNative();
        // mock logic of connector check - data digest
        (address[] memory  tokenAddress, uint[] memory amount) = this.getConnectorData(targets, data);
        // emit log_address(tokenAddress[0]);
        // emit log_uint(amount[0]);
        for (uint i=0; i< data.length; i++){
            if (amount[i] > 0){
                if (tokenAddress[i] != address(0) && tokenAddress[i] != native){
                    // pull tokens if required (this would be from safe)  
                    cheats.prank(0x4A35582a710E1F4b2030A3F826DA20BfB6703C09);
                    IERC20Minimal(tokenAddress[i]).transfer(address(this),amount[i]);
                    // approve to DSA
                    ...
                    // pull ETH - address(this) should have enough alreafy
                }   
            }
        }
        // emit log_uint(address(this).balance);
        assertTrue(address(this).balance > amount[0]);
        uint preCastBalance = address(this).balance;
        IDSA(_account).cast{value: amount[0]}(targets, data, address(0)); // Magic!!
        // check amount is invested
        assertTrue(address(this).balance == preCastBalance - amount[0]);
        assertTrue(_account.balance == amount[0]);
    }
    ...
    function testBasicWithdraw() public {
        ...
        // cast to DSA
        IDSA(_account).cast(targets, data, address(0)); // Magic!!
        (string[] memory targetsW, bytes[] memory dataW) = buildBasicWithdraw();
        IDSA(_account).cast(targetsW, dataW, address(0));
        assertTrue(IERC20Minimal(tokenAddress[0]).balanceOf(address(this)) == amount[0]);
    }
    ...
    function buildBasicDepositNative() public returns (string[] memory _targets, bytes[] memory _data) {
        ...
        uint amtToDeposit = 1e17; // 0.1 MATIC
        // get some DAI from a random DAI-full address to Safe
        // cheats.prank(0x4A35582a710E1F4b2030A3F826DA20BfB6703C09);
        // payable(safeAddress).transfer(amtToDeposit);

        _data[0] = abi.encodeWithSelector(basicDeposit, native, amtToDeposit, 0, 0);
    }
